...
- ASR: 3 candidates (`researching`); the selected candidate is identified as `Const-me/Whisper 1.12.0` with partial C1 evidence, while C0 selects `H:/WhisperCli`/whisper.cpp `large-v3-turbo` as the sole runtime, while FunASR/SenseVoice remain unselected alternatives.
...
- [ ] Independent C1 smoke is partial for the selected ASR candidate: 13/14 checks pass, but C1-ASR-01 fails until an exact wrapper version/source revision is verified; OCR, report, and delivery candidates remain pending.
